...
class Login extends Controller
{
   ...
   function cek_cookie()
   {
      if (isset($_COOKIE["RACSESSID"])) {
         $cookie_value = $this->model("Enc")->dec_2($_COOKIE["RACSESSID"]);
   ...
   function save_cookie($data_user)
   {
      ...
      setcookie("RACSESSID", $cookie_value, time() + 86400, "/");
   }
   ...
   function req_pin()
   {
      ...
      if (isset($cek['otp_active'])) {
         if ($cek['otp_active'] == $today) {
            $cek_deliver = $this->data('Notif')->cek_deliver($hp_input, $today);
            if (isset($cek_deliver['text'])) {
               ...
               if ($res['status']) {
                  $up = $this->db(0)->update('notif', "id_api_2 =  '" . $res['data']['id'] . "'", "id_notif = " . $cek_deliver['id_notif']);
            ...
            $text = $otp . " (" . $cek['nama_user'] . ") - RAC BOOK";
            $hp = $cek['no_user'];
            ...
            if ($res['status']) {
               $do = $this->data('Notif')->insertOTP($res, $today, $hp_input, $text);
